 ...
 * @rongcloud/engine
 * Version: v0.0.1
 * CommitId: cad9ffe7f2ba235eb8c05f053220715c8757068e
 * Date: Tue Nov 10 2020 11:56:43 GMT+0800 (GMT+08:00)
...
(function (global, factory) {
  ...
  `;
    ...
    */
    ...
    const formatWSUrl = (protocol, host, appkey, token, runtime, apiVersion, pid) => {
        return `${protocol}://${host}/websocket?appId=${appkey}&token=${encodeURIComponent(token)}&sdkVer=${apiVersion}&pid=${pid}&apiVer=${ 'normal'}${runtime.connectPlatform ? '&platform=' + runtime.connectPlatform : ''}`;
    };
                    ...
                    const code = yield new Promise((resolve) => {
                        socket.onMessage((data) => {
                            ...
                            if (signal instanceof ConnAckReader) {
                                ...
                                // 立即发送 Ping，1 秒内无响应视为连接失败
                                this._pingResolve = resolve;
                                sendWSData(new PingReqWriter(), socket);
                                setTimeout(() => resolve(ErrorCode$1.TIMEOUT), FIRST_PING_TIMEOUT);
        ...
        handleCometRes(res) {
            ...
            forEach(data, (item) => {
                ...
                if (signal instanceof PublishReader) {
                    ...
                    // 同步消息，等服务确认
                    // this.watcher.signal(signal, ack)
                }
            });
            return true;
        }
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (!this._runtime.useNavi) {
                    const naviInfo = {
                        ...
                        voipServer: '',
                        offlinelogserver: '',
                        backupServer: ((_a = this._customCMP) === null || _a === void 0 ? void 0 : _a.length) ? this._customCMP.join(',') : 'wsproxy.cn.ronghub.com'
                    };
    ...
    */
    ...
    class JSEngine extends AEngine {
        ...
        _receiveSettingNotify(signal) {
            var _a;
            const notice = (_a = this._channel) === null || _a === void 0 ? void 0 : _a.codec.decodeByPBName(signal.data, PBName.UserSettingNotification);
        ...
        */
        ...
        sendMessage(conversationType, targetId, options) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                return {
                    code: ErrorCode$1.SUCCESS,
                    data: transSentAttrs2IReceivedMessage(conversationType, targetId, Object.assign({}, options), pubAck.messageUId, pubAck.timestamp, this.currentUserId)
                };
            });
        }
    ...
    class APIContext {
        ...
        */
        ...
        _pullUserSettings() {
            return __awaiter(this, void 0, void 0, function* () {
                // TODO: 确定拉取时传 0 对后台的影响，是否可以作为纯内存态存储，不存入 Storage
                // logger.error('TODO: 拿取缓存配置，取 version 值')
                const version = 0;
                const res = yield this._engine.pullUserSettings(version);
        ...
        */
        getFileUrl(fileType, uploadMethod, fileName, originName, uploadRes) {
            return __awaiter(this, void 0, void 0, function* () {
    ...
    {
        logger.warn('RCEngineVersion:', "0.0.1");
        logger.warn('VersionCode:', "cad9ffe7f2ba235eb8c05f053220715c8757068e");
    }
   ...
   * Version: v2.6.0
   * CommitId: cad9ffe7f2ba235eb8c05f053220715c8757068e
   * Date: Tue Nov 10 2020 11:56:57 GMT+0800 (GMT+08:00)
   * ©2020 RongCloud, Inc. All rights reserved.
   */
  (function (global, factory) {
      ...
      ***************************************************************************** */
      ...
      function tranToV2Conversation(conversation) {
          const { conversationType, targetId, unreadMessageCount, latestMessage, isTop, notificationStatus } = conversation;
          const v2ReceicedMessage = tranToV2Message(latestMessage);
          const { messageType: objectName, sentTime, receivedStatus } = v2ReceicedMessage;
          return {
              conversationType,
              targetId,
              unreadMessageCount,
              latestMessage: v2ReceicedMessage,
              latestMessageId: v2ReceicedMessage.messageId,
              objectName,
              sentTime,
              isTop,
              notificationStatus,
      ...
      function tranToV2Message(message) {
          let { conversationType, targetId, senderUserId, content, messageType, messageUId, messageDirection, isOffLineMessage: offLineMessage, sentTime, receivedStatus, receivedTime, canIncludeExpansion, disableNotification, expansion } = message;
          ...
          return {
              ...
              receivedTime,
              objectName: messageType,
              messageId: (message.conversationType + '_' + ~~(Math.random() * 0xffffff)),
              sentStatus: SentStatus$1.SENT,
              disableNotification,
              canIncludeExpansion,
              expansion
          };
      ...
      function regMessage(messageType, objectName, isPersited = true, isCounted = true) {
          var _a;
          const define = messageMap[objectName] = (_a = class extends BaseMessage {
                  constructor(content) {
               ...
               * 该类型消息的 MessageType
               */
              _a.MessageType = messageType,
              /**
               * 该类型消息的 ObjectName
               */
              _a.ObjectName = objectName,
              _a);
          return define;
      }
      /**
       * 根据底层接收到的消息数据构建 v2 中的消息结构
       * @param message
       */
      function tranReceivedMessage(message) {
          const { conversationType, targetId, senderUserId, content, messageType, messageUId, messageDirection, isOffLineMessage: offLineMessage, sentTime, receivedTime, canIncludeExpansion, expansion, disableNotification
          // isPersited,
          // isCounted,
           } = message;
          const defined = messageMap[messageType];
          const type = defined ? defined.MessageType : 'UnknownMessage';
          return {
              conversationType,
              targetId,
              senderUserId,
              ...
              canIncludeExpansion,
              expansion,
              receivedStatus: engine.ReceivedStatus.READ,
              sentStatus: SentStatus$1.SENT,
              messageId: (message.conversationType + '_' + ~~(Math.random() * 0xffffff)),
              disableNotification
          };
      ...
      const v2Storage = new engine.AppStorage(runtime$1, 'API-V2');
      ...
      class IMClient {
          ...
          */
          ...
          getConversationList(callback, conversationTypes, count) {
              ...
              this._context.getConversationList(count).then(({ code, data }) => {
                  const conversationList = [];
                  data = data || [];
                  if (code === engine.ErrorCode.SUCCESS) {
                      if (conversationTypes) {
                          data.forEach((item) => {
                              if (conversationTypes.indexOf(item.conversationType) > 0) {
          ...
          getConversation(conversationType, targetId, callback) {
              ...
              this._context.getConversationList(1000).then(({ code, data }) => {
                  if (code === engine.ErrorCode.SUCCESS) {
          ...
          removeConversation(conversationType, targetId, callback) {
              ...
              engine.assert('targetId', targetId, engine.AssertRules.STRING, true);
              engine.assert('callback', callback, engine.AssertRules.CALLBACK);
              this._context.removeConversation(conversationType, targetId).then(code => {
                  if (code === engine.ErrorCode.SUCCESS) {
                      callback.onSuccess();
                  }
                  else {
          ...
          saveTextMessageDraft(conversationType, targetId, draftText) {
              ...
              drafts[`${conversationType}_${targetId}`] = draftText;
              return true;
          }
          /**
           * 获取草稿信息
          ...
          getTextMessageDraft(conversationType, targetId) {
              ...
              return drafts[`${conversationType}_${targetId}`];
          }
          /**
           * 删除草稿
           * @param conversationType
          ...
          clearTextMessageDraft(conversationType, targetId) {
              ...
              const drafts = this._draftMap[userId] || {};
              return delete drafts[`${conversationType}_${targetId}`];
          }
          /**
           * 获取所有会话的消息未读数
           * @description
          ...
          getTotalUnreadCount(callback) {
              engine.assert('callback', callback, engine.AssertRules.CALLBACK);
              this._context.getTotalUnreadCount().then(({ code, data }) => {
                  if (code === engine.ErrorCode.SUCCESS) {
          ...
          getUnreadCount(conversationType, targetId, callback) {
              ...
              engine.assert('targetId', targetId, engine.AssertRules.STRING, true);
              engine.assert('callback', callback, engine.AssertRules.CALLBACK);
              this._context.getUnreadCount(conversationType, targetId).then(({ code, data }) => {
                  if (code === engine.ErrorCode.SUCCESS) {
                      callback.onSuccess(data);
                  }
                  else {
          ...
          getConversationUnreadCount(conversationType, callback) {
              throw engine.todo('getConversationUnreadCount');
          }
          /**
           * 删除指定类型会话，该方法已弃用
           * @deprecated
           ...
           * @param types
           */
          clearConversations(callback, types) {
              throw engine.todo('clearConversations');
          }
          /**
           * 清除指定会话未读数
           * @todo
          ...
          clearUnreadCount(conversationType, targetId, callback) {
              ...
              engine.assert('targetId', targetId, engine.AssertRules.STRING, true);
              engine.assert('callback', callback, engine.AssertRules.CALLBACK);
              this._context.clearUnreadCount(conversationType, targetId).then(code => {
                  if (code === engine.ErrorCode.SUCCESS) {
          ...
          setConversationStatus(conversationType, targetId, status, callback) {
              ...
              this._context.setConversationStatus(conversationType, targetId, isTop, notificationStatus).then(code => {
                  if (code === engine.ErrorCode.SUCCESS) {
                      callback.onSuccess();
                  }
                  else {
          ...
          sendMessage(conversationType, targetId, msg, callback, isMentioned, pushContent, pushData, methodType, options) {
              ...
              const sendOptions = Object.assign(msg, Object.assign(Object.assign({ isMentioned, pushContent, pushData }, options), { messageType: msg.objectName }));
              this._context.sendMessage(conversationType, targetId, sendOptions).then(({ code, data }) => {
                  if (code === engine.ErrorCode.SUCCESS) {
                      callback.onSuccess(tranToV2Message(data));
                  }
                  else {
                      callback.onError(code);
                  }
           ...
           * @param message
           */
          sendRecallMessage(message, callback) {
              ...
              engine.assert('callback', callback, engine.AssertRules.CALLBACK);
              const { conversationType, targetId, messageUId, sentTime } = message;
              this._context.recallMessage(conversationType, targetId, messageUId, sentTime).then(({ code, data }) => {
                  if (code === engine.ErrorCode.SUCCESS) {
                      callback.onSuccess(tranToV2Message(data));
                  }
                  else {
           ...
           * 发送已读回执
           * @description
           * 相较于 2.0 中的方法，该方法需用户指定 messageUId，为保持用户最小改动，messageUId 放在 callback 入参后
           * @param conversationType
           * @param targetId
           * @param callback
           */
          sendReceiptResponse(conversationType, targetId, callback) {
              ...
              if (valObj) {
                  ...
                  const itvId = setInterval(() => {
                      ...
                      var respMsg = new ReadReceiptResponseMessage({ receiptMessageDic });
          ...
          */
           ...
           */
          getHistoryMessages(conversationType, targetId, timestamp, count, callback, order) {
              engine.assert('conversationType', conversationType, engine.AssertRules.NUMBER, true);
              engine.assert('targetId', targetId, engine.AssertRules.STRING, true);
              ...
              this._context.getHistoryMessage(conversationType, targetId, timestamp, count, order).then(({ code, data }) => {
                  if (code === engine.ErrorCode.SUCCESS) {
                      if (data) {
          ...
          deleteRemoteMessages(conversationType, targetId, messages, callback) {
              ...
              engine.assert('callback', callback, engine.AssertRules.CALLBACK);
              this._context.deleteRemoteMessage(conversationType, targetId, messages).then(code => {
                  if (code === engine.ErrorCode.SUCCESS) {
                      callback.onSuccess();
                  }
                  else {
          ...
          clearRemoteHistoryMessages(param, callback) {
              ...
              this._context.deleteRemoteMessageByTimestamp(conversationType, targetId, timestamp).then(code => {
                  if (code === engine.ErrorCode.SUCCESS) {
                      callback.onSuccess();
                  }
                  else {
           ...
           */
          getFileUrl(fileType, fileName, oriname, callback, uploadMethod, uploadRes) {
              engine.assert('fileType', fileType, engine.AssertRules.NUMBER, true);
              ...
              this._context.getFileUrl(fileType, uploadMethod, fileName, oriname, uploadRes).then(callback.onSuccess).catch(callback.onError);
          }
          /* ======================== TODO: RTC 相关接口增加 ======================= */
          ...
          setRTCTotalRes(roomId, message, valueInfo, objectName, callback) {
      ...
      var RongIMClient = {
          /**
            * 初始化 IM SDK
            * @param appkey
            * @param cppProtocol Electron 平台支持的 C++ 协议栈引用，目前暂未实现
            * @param options
            */
          init(appkey, cppProtocol, options = {}) {
              context = engine.APIContext.init(runtime$1, {
                  appkey,
                  apiVersion: "2.6.0",
          ...
          setOnReceiveMessageListener(listener) {
              context.assignWatcher({
                  message: (message) => {
                      try {
                          const v2Message = tranReceivedMessage(message);
                          const day = new Date();
                          ...
                          const { messageType, messageDirection } = v2Message;
                          ...
                          if (messageType === 'ReadReceiptResponseMessage' && dealtime) {
                  ...
                  conversationState: (conversations) => {
                      try {
                          ...
                          conversations.forEach((item) => {
                              let { updatedItems, conversationType, targetId } = item;
          ...
          */
          ...
          reconnect(callback, options) {
              const res = context.reconnect().then(res => {
                  if (res.code === engine.ErrorCode.SUCCESS) {
                      callback.onSuccess(res.userId);
                  }
                  else {
